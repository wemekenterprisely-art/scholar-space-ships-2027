"""internationalopportunities.com - free scholarship listing site."""
import re
from ._shared import get, strip_html

URL = "https://internationalopportunities.com/"

def fetch(timeout: int = 20) -> list[dict]:
    items = []
    try:
        html = get(URL, timeout)
        for m in re.finditer(r'<a[^>]+href="(https://internationalopportunities\.com/[^"]*)"[^>]*>(.*?)</a>', html, re.S):
        ...
        print(f"  [internationalopportunities] scrape error: {e}")
    return items
